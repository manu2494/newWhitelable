...
(function (window, $, channel, Granite, Coral) {
    ...
    var SELECTOR_TEMPLATETYPE_NAME = "[name='./templateType']";
    ...
    var ContentFragmentListController = function () {
        ...
        this.templateTypeContainer= editDialog.querySelector(SELECTOR_TEMPLATETYPE_NAME).parentNode;
    ...
    ContentFragmentListController.prototype.prepareDataSourceRequest = function (url) {
        var componentPath = editDialog.getElementsByClassName(CLASS_EDIT_DIALOG)[0].dataset.componentPath;
        var data = {
            modelPath: modelPath.value
        };
        url = Granite.HTTP.externalize(url) + ".html" + componentPath;
        return $.get({
            url: url,
            data: data
        });
    ...
    ContentFragmentListController.prototype.getUpdatedComponentsHTML = function (callback) {
        ...
        var templateTypeNameRequest = this.prepareDataSourceRequest(this.templateTypeNamePath);
        var self = this;
        // wait for requests to load
        $.when(elementNamesRequest, orderByRequest, templateTypeNameRequest).done(function (elementNamesResult, orderByResult, templateTypeResult) {
            ...
            self.fetchedState = {
                ...
                orderBy: orderBy,
                templateType: templateTypeResult[0]
            };
            callback();
        })
            .fail(function () {
    ...
    ContentFragmentListController.prototype.saveFetchedState = function () {
        ...
        this._updateTemplateTypeByHTML(this.fetchedState.templateType);
    ...
    /**
     * Updates inner html of templateType container.
     *
     */
    ContentFragmentListController.prototype._updateTemplateTypeByHTML = function (html) {
        this.templateTypeContainer.innerHTML = html;
    };
