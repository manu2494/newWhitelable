...
(function (window, $, channel, Granite, Coral) {
    ...
    let CLASS_EDIT_DIALOG = "cmp-contentfragment__editor";

    // field selectors
    let SELECTOR_FRAGMENT_PATH = "[name='./fragmentPath']";
    let SELECTOR_ELEMENT_NAMES_CONTAINER = "[data-element-names-container='true']";
    let SELECTOR_ELEMENT_NAMES = "[data-granite-coral-multifield-name='./elementNames']";
    let SELECTOR_SINGLE_TEXT_ELEMENT = "[data-single-text-selector='true']";
    let SELECTOR_ELEMENT_NAMES_ADD = SELECTOR_ELEMENT_NAMES + " > [is=coral-button]";
    let SELECTOR_VARIATION_NAME = "[name='./variationName']";
    let SELECTOR_TEMPLATETYPE_NAME = "[name='./templateType']";
    let SELECTOR_VARIANT_NAME = "[name='./variant']";
    let SELECTOR_DISPLAY_MODE_RADIO_GROUP = "[data-display-mode-radio-group='true']";
    let SELECTOR_DISPLAY_MODE = "input[name='./displayMode']";
    let SELECTOR_DISPLAY_MODE_CHECKED = "[name='./displayMode']:checked";
    let SELECTOR_PARAGRAPH_CONTROLS = ".cmp-contentfragment__editor-paragraph-controls";
    let SELECTOR_PARAGRAPH_SCOPE = "[name='./paragraphScope']";
    let SELECTOR_PARAGRAPH_RANGE = "[name='./paragraphRange']";
    let SELECTOR_PARAGRAPH_HEADINGS = "[name='./paragraphHeadings']";

    // mode in which only one multiline text element could be selected for display
    let SINGLE_TEXT_DISPLAY_MODE = "singleText";
    let TEMPLATE_DISPLAY_MODE = "template";

    // ui helper
    let ui = $(window).adaptTo("foundation-ui");

    // dialog texts
    let confirmationDialogTitle = Granite.I18n.get("Warning");
    let confirmationDialogMessage = Granite.I18n.get("Please confirm replacing the current content fragment and its configuration");
    let confirmationDialogCancel = Granite.I18n.get("Cancel");
    let confirmationDialogConfirm = Granite.I18n.get("Confirm");
    let errorDialogTitle = Granite.I18n.get("Error");
    let errorDialogMessage = Granite.I18n.get("Failed to load the elements of the selected content fragment");
    ...
    let paragraphControlsTab;

    // keeps track of the current fragment path
    let currentFragmentPath;

    let editDialog;

    let elementsController;

    let currentPageMasterPath;

    // get page properties to get path
    let pagePath = `${CQ.shared.HTTP.getPath().replace('/editor.html', '')}/jcr:content.json`;
    let extPagePath = CQ.shared.HTTP.externalize(pagePath);
    let pageData = CQ.shared.HTTP.get(extPagePath);
    let pageObj = CQ.shared.Util.eval(pageData);
    /**
    ...
    ElementsController.prototype._updateFields = function () {
        ...
        // the Template type field (select)
        this.templateTypeName = editDialog.querySelector(SELECTOR_TEMPLATETYPE_NAME);
        this.templateTypeNamePath = this.templateTypeName.dataset.fieldPath;
    };
    ...
    ElementsController.prototype.prepareRequest = function (displayMode, type) {
        ...
        let data = {
            ...
            templateType: this.templateTypeName.value
        };
        let url;
        let componentPath = editDialog.getElementsByClassName(CLASS_EDIT_DIALOG)[0].dataset.componentPath;
        if (type === "variation") {
            ...
            url = Granite.HTTP.externalize(this.templateTypeNamePath) + ".html" + componentPath;
        }
        else if (type === "variant") {
            url = Granite.HTTP.externalize(this.variantNamePath) + ".html" + componentPath;
        }
    ...
    ElementsController.prototype.testGetHTML = function (displayMode, callback) {
        let elementNamesRequest = this.prepareRequest(displayMode, "elements");
        let variationNameRequest = this.prepareRequest(displayMode, "variation");
        let templateTypeNameRequest = this.prepareRequest(displayMode, "templateType");
        let self = this;
        // wait for requests to load
        $.when(elementNamesRequest, variationNameRequest, templateTypeNameRequest).then(function (result1, result2, result3) {
            let newElementNames = $(result1[0]).find(SELECTOR_ELEMENT_NAMES)[0];
            let newSingleTextSelector = $(result1[0]).find(SELECTOR_SINGLE_TEXT_ELEMENT)[0];
            let newvariationName = $(result2[0]).find(SELECTOR_VARIATION_NAME)[0];
            let newTemplateTypeName = $(result3[0]).find(SELECTOR_TEMPLATETYPE_NAME)[0];
    ...
    ElementsController.prototype.testStateForUpdate = function () {
        ...
        if (this.elementNames && this.elementNames.items.length > 0) {
            ...
            let currentItems = this.elementNames.template.content.querySelectorAll("coral-select-item");
            let newItems = this.fetchedState.elementNames.template.content.querySelectorAll("coral-select-item");
        ...
        if (this.singleTextSelector && this.singleTextSelector.items.length > 0) {
            ...
            // compare the items of the current and new element names fields
            let currentSingleTextItems = this.singleTextSelector.querySelectorAll("coral-select-item");
            let newSingleTextItems = this.fetchedState.singleTextSelector.querySelectorAll("coral-select-item");
        ...
        if (this.templateTypeName.value) {
            // if we're unsetting the current fragment we need to reset the config
            if (!this.fetchedState || !this.fetchedState.templateTypeName) {
                return false;
            }
            // compare the items of the current and new variation name fields
            if (!itemsAreEqual(this.templateTypeName.items.getAll(), this.fetchedState.templateTypeName.items.getAll())) {
                return false;
            }
        }

        if (this.variantName && this.variantName.value) {
            // if we're unsetting the current fragment we need to reset the config
            if (!this.fetchedState || !this.fetchedState.variantName) {
                return false;
            }
            // compare the items of the current and new variation name fields
            if (!itemsAreEqual(this.variantName.items.getAll(), this.fetchedState.variantName.items.getAll())) {
                return false;
            }
        }
    ...
    ElementsController.prototype.saveFetchedState = function () {
        ...
        this._updateTemplateTypeDOM(this.fetchedState.templateTypeName);
        this._updatevariantTypeDOM(this.fetchedState.variantName);
    ...
    ElementsController.prototype.fetchAndUpdateElementsHTML = function (displayMode) {
        let selectNamesRequest = this.prepareRequest(displayMode, "elements");
        let self = this;
        // wait for requests to load
        $.when(selectNamesRequest).then(function (result) {
            self._updateElementsHTML(result);
            self.fetchAndUpdatevariantsHTML(displayMode);
    ...
    /**
     * variants based on templateType.
     */
    ElementsController.prototype.fetchAndUpdatevariantsHTML = function (displayMode) {
        if (displayMode === TEMPLATE_DISPLAY_MODE) {
            this.variantName = editDialog.querySelector(SELECTOR_VARIANT_NAME);
            this.variantNamePath = this.variantName ? this.variantName.dataset.fieldPath : undefined;
            let selectNamesRequest = this.prepareRequest(displayMode, "variant");
            let self = this;
            // wait for requests to load
            $.when(selectNamesRequest).then(function (result) {
                self._updateElementsHTML(result);
    ...
    /**
     * Updates dom of variant Type name select dropdown.
     *
     */
    ElementsController.prototype._updatevariantTypeDOM = function (dom) {
        // replace the variation name select, keeping its value
        if (dom && this.variantName) {
            dom.value = this.variantName.value;
            this.variantName.parentNode.replaceChild(dom, this.variantName);
            this.variantName = dom;
            this.variantName.removeAttribute("disabled");
            this._updateFields();
        }
    };
    ...
    /**
     * Updates dom of Template Type name select dropdown.
     */
    ElementsController.prototype._updateTemplateTypeDOM = function (dom) {
        // replace the variation name select, keeping its value
        if (this.templateTypeName) {
            dom.value = this.templateTypeName.value;
            this.templateTypeName.parentNode.replaceChild(dom, this.templateTypeName);
            this.templateTypeName = dom;
            this.templateTypeName.removeAttribute("disabled");
            this._updateFields();
            // Fetch variations for dynamic dropdown result
            this.templateTypeName.on("change", function () {
                if ($(document).find(SELECTOR_DISPLAY_MODE)[1]) {
                    $(document).find(SELECTOR_DISPLAY_MODE)[1].click();
                }
            });
        }
    };
    ...
    function initialize(dialog) {
        ...
        if(pageObj.contentFragmentMaster && !fragmentPath.value){
            fragmentPath.value= pageObj.contentFragmentMaster;
            currentPageMasterPath =pageObj.contentFragmentMaster;
        }
        ...
        let $radioGroup = $(dialog).find(SELECTOR_DISPLAY_MODE_RADIO_GROUP);
        if(currentPageMasterPath  && elementsController.testStateForUpdate()){
            elementsController.fetchAndUpdateElementsHTML($radioGroup.value, editDialog);
            onFragmentPathChange();
        }
        $radioGroup.on("change", function (e) {
            elementsController.fetchAndUpdateElementsHTML(e.target.value, editDialog);
            updateParagraphControlTabState();
        });
    }
    ...
    function onFragmentPathChange() {
        ...
        if (!fragmentPath.value) {
            let canKeepConfig = elementsController.testStateForUpdate();
            if (canKeepConfig) {
    ...
    function setParagraphControlsState() {
        ...
        let scope = paragraphControls.querySelector(SELECTOR_PARAGRAPH_SCOPE + "[checked]");
        if (scope) {
            // enable or disable range and headings fields according to the scope value
            let range = paragraphControls.querySelector(SELECTOR_PARAGRAPH_RANGE);
            let headings = paragraphControls.querySelector(SELECTOR_PARAGRAPH_HEADINGS);
    ...
    function updateParagraphControlTabState() {
        let displayMode = editDialog.querySelector(SELECTOR_DISPLAY_MODE_CHECKED).value;
